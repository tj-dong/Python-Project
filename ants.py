...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        present = self.place
        distance = 0
        while present.entrance is not None:
            if present.is_hive or distance>self.upper_bound:
                return None
            elif present.bees:
                if self.lower_bound<= distance:
                    return random_bee(present.bees)
            present = present.entrance
            distance +=1
        # END Problem 3 and 4
...
class FireAnt(Ant):
    ...
    def reduce_health(self, amount):
        ...
        if self.health <= amount:
            a = a + self.damage
        i = 0
        while i < len(self.place.bees):
            x = len(self.place.bees)
            self.place.bees[i].reduce_health(a)
            if len(self.place.bees) == x:
                i+=1
...
# The HungryAnt Class
class HungryAnt(Ant):
    """HungryAnt picks a random bee and eats it whole."""
...
class ContainerAnt(Ant):
    ...
    def can_contain(self, other):
        # BEGIN Problem 8a
        "*** YOUR CODE HERE ***"
        return self.ant_contained==None and other.is_container!=True
        # END Problem 8a
...
class TankAnt(ContainerAnt):
    """TankAnt deals 1 damage to all bees in its place while protecting other Ants."""
    ...
    def action(self, gamestate):
        ...
        while i < len(self.place.bees):
            x = len(self.place.bees)
            self.place.bees[i].reduce_health(self.damage)
            if len(self.place.bees) == x:
                i+=1
        if self.ant_contained:
            return self.ant_contained.action(gamestate)
...
class ScubaThrower(ThrowerAnt):
    """ScubaThrower is a more costly but waterproof version of ThrowerAnt."""
        ...
        def action(gamestate):
            if target.is_slowed:
                ...
                if gamestate.time % 2 == 0:
                    return Bee.action(target, gamestate)
            else:
                 return Bee.action(target, gamestate)
